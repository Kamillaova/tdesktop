...
void Photo::paintUserpicFrame(
	...
	recordStreamedContentRect(
		p,
		context,
		QRect(photoPosition, QSize(width(), height())));
...
void Photo::repaintStreamedContent() {
	if (_streamed && !_streamed->frozenFrame.isNull()) {
		...
		return;
	} else if (_streamed && _streamed->repaintPending) {
		return;
	}
	if (!_streamed) {
		repaint();
...
void Photo::recordStreamedContentRect(
		const Painter &p,
		const PaintContext &context,
		QRect rect) const {
	if (!_streamed) {
